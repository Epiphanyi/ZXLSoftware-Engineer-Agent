...
def run_command(command: str) -> Dict[str, Any]:
    """Run a shell command."""
    try:
        # Security check: prevent dangerous commands if possible, but for a dev tool, flexibility is key.
        # Maybe block 'rm -rf /' or similar if easy, but user is in control.
...
[bold green]Ready to architect and build your software projects![/bold green]
    ...
    def process_user_input(self, user_input: str):
        """Dispatch to appropriate stream method."""
        if self.provider in ['openai', 'deepseek', 'qwen']:
            self.stream_openai_response(user_input)
        else:
            self.stream_gemini_response(user_input)

    def stream_openai_response(self, user_input: str):
        """Stream response from OpenAI/DeepSeek with function calling support."""
        try:
            # Add user message to history
            self.conversation_history.append(ConversationMessage("user", user_input))
            ...
            with Progress(SpinnerColumn(), TextColumn("[blue]🤖 Analyzing and planning..."), console=self.console) as progress:
                ...
                try:
                    ...
                    self.console.print(f"[red]❌ OpenAI API Request failed: {e}[/red]")
                    progress.remove_task(task)
                    return
                ...
                if delta.tool_calls:
                    for tc in delta.tool_calls:
                        ...
                        if tc.function.name:
                            ...
                            self.console.print(f"\n[green]📞 Function call detected: {tc.function.name}[/green]")
            ...
            if full_content:
                self.conversation_history.append(ConversationMessage("assistant", full_content))
                ...
                for index, tool_data in tool_calls_buffer.items():
                    try:
                        ...
                        try:
                            ...
                            self.console.print(f"[red]❌ Failed to parse arguments for {tool_name}[/red]")
                            continue
                            
                        # Display
                        param_display = ', '.join(f'{k}={str(v)[:50]}...' if len(str(v)) > 50 else f'{k}={v}' for k, v in parameters.items())
                        self.console.print(f"[cyan]▶ {tool_name}({param_display})[/cyan]")
                        
                        result = self.execute_tool(tool_name, parameters)
                        self.display_tool_result(tool_name, result)
                        
                        # Add follow up (mimicking Gemini behavior)
                        if result.get("success"):
                             follow_up = f"Successfully executed {tool_name}."
                        else:
                             follow_up = f"Error executing {tool_name}: {result.get('error')}"
                        
                        self.conversation_history.append(ConversationMessage("assistant", follow_up))
            ...
            self.console.print(f"[red]❌ Error in OpenAI stream: {e}[/red]")
